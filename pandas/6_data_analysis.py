...
# * 판매 금액(Sales_Amount) 컬럼의 결측치를 지역(Region)별 평균으로 변경
sales_by_region = df.groupby('Region')['Sales_Amount']

print(sales_by_region.median())

df['Sales_Amount'] = sales_by_region.transform(lambda x: x.fillna(x.mean()))
...
# 판매 금액 데이터 중 상한선을 초과하는 값을 상한선으로 변경
print('* ---- 이상치 처리 전 ---- *')
...
# * 매출 등급 생성 (Price_Grade)
#   판매 금액이 전체 판매 금액 평균보다 높으면 'High', 그렇지 않으면 'Low'
...
# * 단순히 평균, 합계가 아닌 복합적인 통계(그룹 별 비교, 정규화, 다차원 집계 등)를 사용하여 데이터의 숨겨진 패턴이나 관계를 파악하는 과정 => 심층 분석

# * 그룹별 복합 통계 *
#   agg: 그룹화된 데이터에 대해 여러 개의 집계 함수를 한 번에 적용하여 요약 통계를 생성하는 함수
#       groupby 결과에 대해 sum, mean, max, std 등 집계 함수를 적용
...
# df.round(소수점_자리): 해당 소숫점 자리까지 반올림
...
# * 지역 별 고객 평점 평균을 기준으로 개별 평균 차이(Rating_Diff)
# * 지역 별 고객 평점 평균
...
# 지역, 고객 평점, 평균대비 평점 차이 출력
print(df[['Region', 'Customer_Rating', 'Rating_Diff']])


# * 피벗 테이블
#   pivot_table(df, index=행분류기준컬럼, columns열분류기준컬럼, values=집계대상값, aggfunc=적용할집계함수)
...
#                 groupby와 agg를 조합간 것과 유사하지만, 가로/세로 축으로 펼쳐서 보여주는 특징을 가짐
...
# 결측치 대체값 걸정 pivot_table(..., fill_value=대체값)